...
def disease_dict(request):
    ...
    if query:
        diseases = disease.objects.filter(NAME__icontains = query)  # 질병명 검색
        if diseases:
            return render(request, 'disease_dict/disease_dict.html', {'diseases':diseases})
        else:
            return render(request, 'disease_dict/disease_dict.html', {'error_message':'결과가 없습니다.'})
    else:
        return render(request, 'disease_dict/disease_dict.html', {'error_message':'질병명을 입력하세요.'})
