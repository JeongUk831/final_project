...
def q_symptom(self) :
    ...
    # 체크박스를 표시할 레이아웃을 생성함
    layout = [[sg.Text("아픈 부위를 선택하세요:")]]
    for question in question_list:
        layout.append([sg.Checkbox(question["name"], key=question["name"])])
    layout.append([sg.Button("확인"), sg.Button("취소")])

    # 체크박스 창을 실행하고 사용자의 선택에 따라 질문에 대한 답변을 저장함
    window = sg.Window("증상 선택", layout)
    while True:
        event, values = window.read()
        if event in (sg.WIN_CLOSED, "취소"):
            break
        #체크박스에 따라 if 문 생성
        if values["소화계"]:  # 소화계 체크박스가 선택되어 있는 경우
        ...
        #체크박스에 따라 if 문 생성            
        for question in question_list:
            answer_dict[question["name"]] = values[question["name"]]
        break
    window.close()



    # 결측치가 가장 많은 컬럼부터 차례대로 삭제함
    na_ratio = filtered_df.isna().sum() / len(filtered_df)
    na_ratio = na_ratio.sort_values(ascending=False)
    drop_columns = []
    for column, ratio in na_ratio.items():
        if len(filtered_df.columns) - len(drop_columns) > 10:
